...
Vagrant.configure("2") do |config|
  ...
  config.ssh.host = vm_ip #guest OS IP for ssh to connect
  ...
  config.vm.provider "virtualbox" do |vb|
    vb.memory = "4096"
    vb.cpus = 2
    vb.customize ["modifyvm", :id, "--uartmode1", "disconnected"] # Disable serial port which can prevent connection issues during VM startup
  end
